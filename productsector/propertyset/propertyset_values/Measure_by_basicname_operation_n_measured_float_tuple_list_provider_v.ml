(** {3 Measure_by_basicname_operation_n_measured_float_tuple_list_provider_v} *)

(** {6 Documenting_for_value_v} *)

let documentation () = 
  [
   "Current : VPRO:Measure_by_basicname_operation_n_measured_float_tuple_list_provider_v";
...
let build (bna_opt, mea_hom_tl) =
  ...
  let nam_opt = Basicname_v.string_off bna_opt in
	...
	Coordinate_operation_measure_v.measure_length_of_measured_float_tuple_duo mea_hom_td
	  
    | "perimeter" ->

	let mea_hom_tt = Trio_v.trio_of_list mea_hom_tl in
	Coordinate_operation_measure_v.measure_perimeter_of_measured_float_tuple_trio mea_hom_tt
