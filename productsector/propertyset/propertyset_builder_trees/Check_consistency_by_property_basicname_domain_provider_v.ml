(** {3 Check_consistency_by_property_basicname_domain_provider_v} *)
...
let build bna_dom =
  let bna_pbd = Property_basicname_domain_by_unit_provider_v.provide () in
  if bna_pbd <> bna_dom 
  then
    Error_messages_v.print_fatal_error nam_mod "build"
      (Format.sprintf "Argument Domain basicname were %s" (Basicname_v.name bna_pbd))
      (Basicname_v.name bna_dom )
      "Check"
  else ();
;;
