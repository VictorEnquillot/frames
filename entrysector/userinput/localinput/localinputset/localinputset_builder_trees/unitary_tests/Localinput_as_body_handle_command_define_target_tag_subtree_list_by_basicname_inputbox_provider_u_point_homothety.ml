...
testing "Localinput_as_body_handle_command_define_target_tag_subtree_list_by_basicname_inputbox_provider_v with
...
(* toplevel 
   #use "Localinput_as_body_handle_command_define_target_tag_subtree_list_by_basicname_inputbox_provider_u_point_homothety.ml";; 
...
let tag_pre_tre =
  ...
  let pre_nod t = Localinput_symbol_v.is_input_keyword_target (Tag_v.entity_off_tag t)in
  Tree_v.has_a_topson_leaf_and_a_topson_node_of_leaf_predicate_of_node_predicate_off_tree pre_lea pre_nod 
;;

let tag_inp_tar_stl =  List.filter tag_pre_tre tag_inp_com_stl;;

test_number 1 (
(tag_inp_tar_stl : Localinput_symbol_t.input_symbol Tag_t.tag Tree_t.tree list) =
[Tree_t.Inner
