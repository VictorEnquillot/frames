...
(* define subtree *)
  ...
  let tag_loi_def_tl = List.filter 
      (Tree_v.has_topson_of_leaf_predicate_of_tree 
         (fun t -> Localinput_tag_v.string_off t = nam_iev)) 
      tag_loi_def_stl 
 ;;

  let tag_loi_def_st = List_v.element_off_one_element_list tag_loi_def_tl;;
...
=
  ...
  let sym_inp_subp_st = Tree_v.map Tag_v.entity_off_tag tag_loi_subp_st;;
