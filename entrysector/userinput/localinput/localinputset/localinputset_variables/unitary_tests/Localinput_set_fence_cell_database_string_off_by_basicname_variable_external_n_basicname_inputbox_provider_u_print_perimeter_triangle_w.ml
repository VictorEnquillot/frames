open Make_test_v;;

testing "Localinput_set_fence_cell_database_string_off_by_basicname_variable_external_n_basicname_inputbox_provider_v with
        Localinput_set_fence_cell_database_string_off_by_basicname_variable_external_n_basicname_inputbox_provider_u_print_perimeter_triangle_w.ml";;

(* define Con_bc *)
(*        context *)
(*          databox "segment_BC" *)
(*          database "db1points" *)
(*          domain "figure" *)
(*          sector "internal" *)
(*          end -- context *)
(*        end -- define *)

(* define Ope_len *)
(*        operator  *)
(*          type [ "operator" "set" "body" "creation" "tomeasure" "constructor"] *)
(*          operation "length" *)
(*          ondomain "figure"   *)
(*          end -- operator *)
(*        end -- define *)

(* define Seg_bc *)
(*        external  *)
(*          type [ "figure" "set" "body" "segment" "constructor" ] *)
(*          external_name "BC" *)
(*          external_context Con_bc *)
(*          end -- external *)
(*        end -- define *)

(* define Len_bc *)
(*        target  *)
(*          type [ "property" "operand" "body" "target" "measured" "constructor" ] *)
(*          target_operator Ope_len *)
(*          target_operand Seg_bc *)
(*          end -- target *)
(*        end -- define *)

(* define Uni_len *)
(*        external  *)
(*          type [ "units" "length" "bohr" ] *)
(*          end -- external *)
(*        end -- define *)

(* print Len_bc *)
(*       units [ Uni_len ] *)
(*       end -- print *)

(* stop *)


(* Deleting Registers *)


(* Tracing *)

(* toplevel 
   #use "Localinput_set_fence_cell_database_string_off_by_basicname_variable_external_n_basicname_inputbox_provider_u_print_perimeter_triangle_w.ml";; 

*)

open Localinput_set_fence_cell_database_string_off_by_basicname_variable_external_n_basicname_inputbox_provider_v;;

let nam_var_ext = "Seg_bc";;
let bna_var_ext = Basicname_v.basicname_variable_external_of_string nam_var_ext;;

let nam_ibo = "Print_length_segment_bc";;
let bna_ibo = Basicname_v.basicname_inputbox_of_string nam_ibo;;

let tag_loi_lcd_stl =
  Localinput_as_set_body_handle_command_creation_define_tag_entity_external_subtree_list_by_basicname_inputbox_provider_v.provide
    bna_ibo
;;

let tag_loi_lcd_st = List_v.only_element_of_predicate_off_list 
      (fun t -> 
	(Localinput_tag_v.string_off (Tree_v.root_off_tree t) = nam_var_ext ) 
     )
      tag_loi_lcd_stl;;

let tag_loi_lee = Tree_v.only_node_of_node_predicate_off_tree 
      (fun (s, i) -> 
	(Localinput_symbol_v.is_localinput_set_fence_variable_name_entity_external_constructor s)
      )
      tag_loi_lcd_st;;

let tag_lee = (* Coerce Down *)
    Localinput_set_fence_variable_name_entity_external_tag_v.localinput_set_fence_variable_name_entity_external_tag_off_localinput_tag 
    tag_loi_lee;;

let tag_lec = 
    Localinput_set_fence_cell_external_context_tag_by_localinput_set_fence_variable_name_entity_external_tag_provider_v.provide 
      tag_lee;;

test_number 1 (
(tag_lec :
  Localinput_set_fence_cell_external_context_symbol_t.localinput_set_fence_cell_external_context_symbol
  Tag_t.tag ) =
  (Localinput_set_fence_cell_external_context_symbol_t.Localinput_set_fence_cell_external_context_constructor
    "Con_bc",
   [3; 2; 2; 2; 3; 42; 1; 22; 1])
);;

let tag_lnc =
    Localinput_set_fence_variable_name_context_tag_by_localinput_set_fence_cell_external_context_tag_provider_v.provide
      tag_lec;;

test_number 2 (
(tag_lnc :
  Localinput_set_fence_variable_name_context_symbol_t.localinput_set_fence_variable_name_context_symbol
  Tag_t.tag ) =
  (Localinput_set_fence_variable_name_context_symbol_t.Localinput_set_fence_variable_name_context_constructor
    "Con_bc",
   [1; 2; 1; 42; 1; 22; 1])
);;

let tag_lcd =
    Localinput_set_fence_cell_database_tag_by_localinput_set_fence_variable_name_context_tag_provider_v.provide
    tag_lnc;;

test_number 3 (
(tag_lcd :
  Localinput_set_fence_cell_database_symbol_t.localinput_set_fence_cell_database_symbol
  Tag_t.tag ) =
  (Localinput_set_fence_cell_database_symbol_t.Localinput_set_fence_cell_database_constructor
    "figure",
   [3; 2; 2; 2; 1; 42; 1; 22; 1])
);;

let nam_dba = Localinput_set_fence_cell_database_tag_v.string_off tag_lcd;;

test_number 4 (
nam_dba = provide (bna_var_ext, bna_ibo)
);;


