...
let documentation () = 
  [
   "Current : VARLOI:Localinput_as_set_body_handle_command_tag_subtree_list_by_basicname_inputbox_register_v";
...
let register : 
    (Basicname_t.basicname,
     (Localinput_tag_t.localinput_tag Tree_t.tree) list
    ) 
    Register_t.register = 
...
let name_of_value val_ =
  List_v.name
    (Tree_v.name Localinput_tag_v.fullname) 
    val_
;;
