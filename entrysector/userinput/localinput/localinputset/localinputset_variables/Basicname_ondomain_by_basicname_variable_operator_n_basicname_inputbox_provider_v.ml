(** {3 Basicname_ondomain_by_basicname_variable_operator_n_basicname_inputbox_provider_v} *)

(** {6 Documenting} *)

let documentation () = 
  [
   "Current : VARLOI:Basicname_ondomain_by_basicname_variable_operator_n_basicname_inputbox_provider_v";
   ...
   "Register : VARLOI:Basicname_ondomain_by_basicname_variable_operator_n_basicname_inputbox_register_v";
   "Needed-by :"; 
   "What-is-it : for a given Basicname Variable_operator it provides its Basicname Ondomain from its Basicname Variable_operator";
   "How-is-it-done : by extracting its Basicname_context_context string_off from its Define B-Subtree";
   "How-is-it-done : and extracting Leaf Basicname_ondomain_constructor from it";
   "Author : François Colonna 26 avril 2017 at 10:14:40+02:00";
 ]
;;
...
let build (bna_var_ope, bna_ibo) =
  ...
  let tag_loi_dbo = Tree_v.only_node_of_node_predicate_off_tree
      (fun (s, i) -> 
        Localinput_symbol_v.is_localinput_set_fence_cell_ondomain_symbol_off_localinput_symbol s)
      tag_loi_lcd_st 
  in
  
  let sof_loi_dbo = Localinput_tag_v.string_off tag_loi_dbo in

  Basicname_v.basicname_ondomain_of_string sof_loi_dbo
;;
...
let store (bna_var_ope, bna_ibo) val_ =
  ...
  Basicname_ondomain_by_basicname_variable_operator_n_basicname_inputbox_register_v.store nam_mod (bna_var_ope, bna_ibo) val_;
...
let retrieve (bna_var_ope, bna_ibo) =
  ...
  let pro_cpu = Management_v.entering_of_module_name_of_function_name nam_mod nam_fun in
  let result = Basicname_ondomain_by_basicname_variable_operator_n_basicname_inputbox_register_v.retrieve nam_mod (bna_var_ope, bna_ibo) in
  Management_v.exiting_of_process_times_of_module_name_of_function_name pro_cpu nam_mod nam_fun;
  result
;;
...
let provide_without_trace (bna_var_ope, bna_ibo) =
  if Basicname_ondomain_by_basicname_variable_operator_n_basicname_inputbox_register_v.is_stored (bna_var_ope, bna_ibo)
...
(* done with do_provider_with_register.sh Basicname_ondomain_by_basicname_variable_operator_n_basicname_inputbox_provider_v.ml force on mardi 25 avril 2017, 15:59:44 (UTC+0200) *)
