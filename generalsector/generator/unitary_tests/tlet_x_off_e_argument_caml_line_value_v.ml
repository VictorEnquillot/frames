...
testing "Let_b_off_t_argument Caml_line_value_v";;

(* toplevel 
   #use "tlet_b_off_t_argument_caml_line_value_v.ml";; 
 *)
...
let mrk_cnt_atsd = Cnm_v.make smb_cnt_atsd dep_cnt_atsd;;
...
let (smb_cnf, dep_cnf) = Clf_v.up_constructor_tag_off_constructor_path pth_cnt_atsd;;
let smb_enu = Cns_v.entity_symbol_off_constructor_symbol smb_cnf;;
let ecmf = Ecm_v.make smb_enu smb_cat smb_cfi;;

let (smb_cnx, dep_cnx) = List.hd pth_cnt_atsd;;
let smb_enx = Cns_v.entity_symbol_off_constructor_symbol smb_cnx;;
let ecmx = Ecm_v.make smb_enx smb_cat smb_cfi;;
...
testi 3 (
Clv_v.make_b_off_t_t ecmx ecmf
(* : Caml_line_value_t.caml_line_value *)
=
"Ats_v.atom_twotied_single_double_symbol_off_atom_twotied_symbol smb_atd"
);;
