(** {3 The Provider for Module Abbreviation for each entitycategorycodefile.} *)

let nam_cod = "abbreviation_module_by_entitycategorycodefile_provider_v.ml";;
...
let nam_reg = "abbreviation_module_by_entitycategorycodefile";;

let abbreviation_module_by_entitycategorycodefile : 
    (Ecc_t.entitycategorycodefile, string) Reg_t.register = Reg_v.make 137;;

let store ecc abb_ecc =
  if (Reg_v.is_value_stored abbreviation_module_by_entitycategorycodefile abb_ecc)
      && (abb_ecc <> "") 
  then 
    ...
    try Reg_v.store abbreviation_module_by_entitycategorycodefile ecc abb_ecc
    with Failure ("Already_stored:Reg_v.store") ->
      failwith ("Already_stored:Map_v.store")
;;

let retrieve ecc =
  try Reg_v.retrieve abbreviation_module_by_entitycategorycodefile ecc 
...
let build = function 
  ...
  | ecc ->
      ...
      Format.sprintf "%s_%s" 
	abb_enc (Ecc_v.codefile_suffix_name ecc )
;;
...
let provide ecc =
  if Reg_v.is_stored abbreviation_module_by_entitycategorycodefile ecc 
  then retrieve ecc
