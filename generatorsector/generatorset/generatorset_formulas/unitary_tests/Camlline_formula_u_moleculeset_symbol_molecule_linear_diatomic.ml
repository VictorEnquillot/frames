...
testing "Camlline_formula_v with
   Camlline_formula_u_moleculeset_formula_molecule_linear_diatomic.ml";;

(* toplevel 
   #use "Camlline_formula_u_moleculeset_formula_molecule_linear_diatomic.ml";;
...
=
(Entity_symbol_t.Entity_proper "molecule_linear_diatomic", [8; 1; 3])
);;
...
tag_usf_typ
  =
(Usagefile_symbol_t.Implementation, [1; 1; 8; 1; 3])
);;
...
tag_usf_val
  =
(Usagefile_symbol_t.Implementation, [1; 2; 8; 1; 3])
);;
...
tag_cap_nam_1 
=
(Camlparagraph_symbol_v.let_name_equal_function_for_symbol, [2; 3; 1; 2; 8; 1; 3])
);;
