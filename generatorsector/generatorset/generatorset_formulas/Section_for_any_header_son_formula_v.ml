...
let documentation () = 
  [
   "Needs : IGNR:";
   "Current : FGNR:Section_for_any_header_son_formula_v";
   "Needed-by : FGNR:";
...
let symbol_of_formula for_shs = 
  match for_shs with
  | Section_for_any_header_son_formula_t.Section_for_any_header_son_empty ->
...
let name for_shs =
  ...
  Section_for_any_header_son_symbol_v.name sym_shs
;;
(* ***
let longname for_shs =
  let sym_shs = symbol_of_formula for_shs in
  Section_for_any_header_son_symbol_v.longname sym_shs
;;

let string_off for_shs =
  let sym_shs = symbol_of_formula for_shs in
  Section_for_any_header_son_symbol_v.string_off sym_shs
;;
...
let interface sym_shs =  (* Basic Formula read from dominset_formulas.set as a DAS *)
  match sym_shs with 
  | Section_for_any_header_son_symbol_t.Section_for_any_header_son_empty ->
      []
;;
...
let builder_tag_datastructure sym_shs soi_sec =
  let sym_gro_l = interface sym_shs in
...
let store sym_shs soi_sec =
  let tag_gro_l = builder_tag_datastructure sym_shs soi_sec in
...
let group_tag_list_off_section_for_any_header_son_formula for_shs = 
  match for_shs with
  | Section_for_any_header_son_formula_t.Section_for_any_header_son_empty ->
      []
;;

let camlline_tag_list_off_section_node_formula for_shs =
  let tag_gro_l = 
    group_tag_list_off_section_for_any_header_son_formula
      for_shs
  in
...
let map f for_shs = 
  let tag_gro_l = group_tag_list_off_section_for_any_header_son_formula for_shs in
...
let map2 f for_shs_1 for_shs_2  = 
  let tag_gro_l_1 = group_tag_list_off_section_for_any_header_son_formula for_shs_1 in
  let tag_gro_l_2 = group_tag_list_off_section_for_any_header_son_formula for_shs_2 in
  List.map2 f tag_gro_l_1 tag_gro_l_2
;;

let iter2 f for_shs_1 for_shs_2 = 
  let tag_gro_l_1 = group_tag_list_off_section_for_any_header_son_formula for_shs_1 in
  let tag_gro_l_2 = group_tag_list_off_section_for_any_header_son_formula for_shs_2 in
