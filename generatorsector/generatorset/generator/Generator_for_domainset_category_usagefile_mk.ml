...
let nam_cod = "Do_figureset_formulas_files_mli_mk.ml";;

(* toplevel
  #use "Do_figureset_formulas_files_mli_mk.ml";;
...
let sym_dom = Domainset_symbol_v.make nam_dom;;
let sip_dom = [2];;
let gss_dom = Generatorset_symbol_v.generatorset_symbol_of_domainset_symbol sym_dom;;
Generatorset_symbol_by_sole_index_register_v.store_force sip_dom gss_dom;;
let tag_dom = Tag_v.make sym_dom sip_dom;;
let for_dom = Domainset_formula_v.retrieve tag_dom;;

let tag_cat = Doublet_list_v.find_if_left sel_cat for_dom;;

let nameoffile_makefile nam_dir_dom tag_cat =
...
let write_makefile_top_of_oc oc =
...
let write_makefile_of_category_tag tag_cat =
  let nof = nameoffile_makefile nam_dir_dom tag_cat in
  ...
  then
    ...
    begin
      ...
      let sym_ent_ft = 
	Entity_symbol_fulltree_by_domainset_tag_provider_v.provide tag_dom
      in
      let sym_n_dep_ent_l = 
	List.rev (
...
write_makefile_of_category_tag tag_cat;
