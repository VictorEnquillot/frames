...
let nam_cod = "Do_naturalset_tags_files_mli_mk.ml";;

(* toplevel
  #use "Do_naturalset_tags_files_mli_mk.ml";;
*)
...
let nam_dir_dom = nam_dir_fra ^ nam_dom ^ "/generator/";;
...
let write_bottom_of_oc oc =
  let bot_l = [
    ...
    "#naturalset_symbol_by_sole_index_provider_v.mli \\";
    "";
  ]
  in
...
let write tag_cat =
  ...
  let sym_ent_ft = 
    Entity_symbol_constructor_tree_by_domain_symbol_provider_v.provide tag_dom
  in
  let sym_n_dep_ent_l = 
    List.rev (
