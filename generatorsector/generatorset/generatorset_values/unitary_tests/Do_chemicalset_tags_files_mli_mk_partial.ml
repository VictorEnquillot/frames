...
let nam_cod = "Do_chemicalset_tags_files_mli_mk_partial.ml";;

(* toplevel
  #use "Do_chemicalset_tags_files_mli_mk_partial.ml";;
*)
...
let write_tag_t_of_oc_of_entity_symbol_n_depth oc sym_n_dep =
...
let write_bottom_of_oc oc =
  let bot_l = [
    ...
    "#chemicalset_tag_by_sole_index_provider_v.mli \\";
    "";
  ]
  in
...
let write tag_cat =
  ...
  let partial_sym_n_dep_ent_l =
	...
	(select_entity_name_n_depth "atom" 1 10 d)
      ||
	(select_entity_name_n_depth "halbridge" 1 10 d)
      ||
  ...
  in
  write_top_of_oc oc;
  List.iter (write_tag_t_of_oc_of_entity_symbol_n_depth oc) partial_sym_n_dep_ent_l;
