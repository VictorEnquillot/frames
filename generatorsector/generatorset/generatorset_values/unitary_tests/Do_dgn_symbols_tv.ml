(** {3 Creates  Dgn_*symbol_[tv].ml} *)

let nam_cod = "Do_dgn_symbols_files_mli_mk.ml";;

(* Deleting Registers *)

Register_v.delete Generatorset_symbol_by_sole_index_register_v.register;;
...
(* toplevel
  #use "Do_dgn_symbols_tv.ml";;

*)

let nam_dom = "figure";;  (* Improve Still a pb with databaseset domain *)
let nam_cat = "symbol";;

let nam_usf = "implementation_for_" ^ nam_cat;;
...
let write_t nam_dir_cat tag_enp =
  ...
  then
    begin 
      ...
      Format.fprintf Format.err_formatter "rm %s@.# Cpu %f Total %f@."
