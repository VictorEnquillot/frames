let nam_cod = "Do_naturalset_formulas_tv.ml";;

(* toplevel 
   #use "Do_naturalset_formulas_tv.ml";;
  ...
  then
    ...
    begin
      ...
      Format.fprintf Format.err_formatter "Cpu %f Total %f module T %s@." 
        (out_cpu -. in_cpu) out_cpu (Entity_symbol_v.string_off (Tag_v.entity_off_tag tag_ent) );
    end
;;
