(** {3 Provides a Description_string for each Entity_name *)

let nam_cod = "Description_string_by_entity_trail_provider_v.ml";;

(** {6 Documenting} *)

let documentation () = 
  [
   "Needs : AGNR:Constructor_basename_description_string_doublet_list_by_category_trail_provider_v";
   ...
   "What-is-it : it is the string following the = sign in record";
   "Remark : Symbols have entity names as description (kind type or type_private)";
   "Remark : Formulas have Datastructure as description (kind builders)";
   "Remark : For each entity_name Description is kept in the original string.";
...
let nam_reg = "description_string_by_entity_trail_register";;

let description_string_by_entity_trail_register : 
    (Trail_t.trail, 
     string) 
...
let retrieve tra_ent =
  try Register_v.retrieve description_string_by_entity_trail_register tra_ent
  with Failure ("Not_stored:Register_v.ml:retrieve") ->
    failwith ("Not_stored:"^nam_cod^":retrieve")
;;  

let store tra_ent str =
  try Register_v.store description_string_by_entity_trail_register tra_ent str
...
let is_stored tra_ent =
  Register_v.is_stored description_string_by_entity_trail_register tra_ent
;;

let build_n_store tra_ent =
  let tra_cat = Trail_v.trail_of_key_off_trail "entity" tra_ent in 
  let nam_ent_str_con_dol = 
    Constructor_basename_description_string_doublet_list_by_category_trail_provider_v.provide 
      tra_cat
  in
...
let provide tra_ent =
  if Register_v.is_stored 
      description_string_by_entity_trail_register 
      tra_ent
