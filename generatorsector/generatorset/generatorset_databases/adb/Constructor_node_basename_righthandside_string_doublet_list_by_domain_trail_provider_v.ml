(** {3 Constructor_node_basename_righthandside_string_doublet_list_by_domain_trail_provider_v *)
...
let documentation () = 
  [
   ...
   "Current : AGNR:Constructor_node_basename_righthandside_string_doublet_list_by_domain_trail_provider_v";
   "Needed-by : ";
   ...
   "How-is-it-done : by specifying \"constructors\" kind_basename";
 ]
;;
...
let nam_reg = "constructor_node_basename_righthandside_string_doublet_list_by_domain_trail_register";;

let constructor_node_basename_righthandside_string_doublet_list_by_domain_trail_register : 
    (Trail_t.trail, 
     (string, string) 
...
let retrieve tra_dos =
  try Register_v.retrieve 
      constructor_node_basename_righthandside_string_doublet_list_by_domain_trail_register
      tra_dos
...
let build tra_dos =
  let tri_ckr_l = 
    Constructor_basename_kind_basename_righthandside_string_triplet_list_by_category_trail_provider_v.provide
      tra_dos
  in
  let ban_con_l = List.map (fun (c, k, rhs) -> (c, rhs) ) tri_c_l in
  let cmp (a, b) (x, y) = (String.compare a x) in

  List.sort cmp ban_con_l 
;;
