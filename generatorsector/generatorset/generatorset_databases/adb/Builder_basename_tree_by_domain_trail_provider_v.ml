(** {3 A Provider for the Builder_tree for a Domainset}  *)

let nam_cod = "Builder_basename_tree_by_domain_trail_provider_v.ml";;
...
let documentation () = 
  [
   ...
   "Needs : AGNR:Builder_root_basename_list_by_constructor_leaf_trail_provide_v";
   "Needed-by : ";
...
let nam_reg = "builder_basename_tree_by_domain_trail_register";;

let builder_basename_tree_by_domain_trail_register : 
    (Trail_t.trail, 
     string Tree_t.tree) 
    Register_t.register = Register_v.make 137;;

let store tra_dos wor_t =
  try Register_v.store builder_basename_tree_by_domain_trail_register 
      tra_dos 
      wor_t
...
let is_stored tra_dos =
  Register_v.is_stored builder_basename_tree_by_domain_trail_register tra_dos
;;

let retrieve tra_dos =
  try Register_v.retrieve 
      builder_basename_tree_by_domain_trail_register 
      tra_dos
...
let build tra_dos =
  ...
  let nam_bui_roo_l = 
    Builder_root_basename_list_by_constructor_leaf_trail_provide_v.provide 
      tra_dos in
in
  let tra_bui_roo_l =
    List.map 
...
let provide tra_dos =
	...
	"Check"
      ...
      begin
	if Register_v.is_stored builder_basename_tree_by_domain_trail_register tra_dos 
