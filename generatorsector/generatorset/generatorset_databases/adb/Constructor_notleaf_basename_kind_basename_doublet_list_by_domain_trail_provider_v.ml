(** {3 Constructor_notleaf_basename_kind_basename_doublet_list_by_domain_trail_provider_v *)
...
let documentation () = 
  [
   ...
   "Current : AGNR:Constructor_notleaf_basename_kind_basename_doublet_list_by_domain_trail_provider_v";
   "Needed-by : ";
   "Definition : a Constructor_node is a Node of the Constructor Tree (CT)";
   "Definition : the Kind_name is the string at the right of the = sign";
...
let nam_reg = "constructor_notleaf_basename_kind_basename_doublet_list_by_domain_trail_register";;

let constructor_notleaf_basename_kind_basename_doublet_list_by_domain_trail_register : 
    (Trail_t.trail, 
     (string, string) 
...
let retrieve tra_dos =
  try Register_v.retrieve 
      constructor_notleaf_basename_kind_basename_doublet_list_by_domain_trail_register
      tra_dos
...
let build tra_dos =
  ...
  let tri_ckr_l = 
    Entity_basename_kind_basename_righthandside_string_triplet_list_by_category_trail_provider_v.provide
      tra_cat
  in
  let dou_cr_l = List.map (fun (c, k, rhs) -> (c, k) ) tri_ckr_l in
