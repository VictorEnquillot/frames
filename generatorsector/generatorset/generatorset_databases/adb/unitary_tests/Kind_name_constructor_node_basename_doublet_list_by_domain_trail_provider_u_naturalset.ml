open Make_test_v;;

testing "Kind_name_constructor_node_basename_doublet_list_by_domain_trail_provider_v
   Kind_name_constructor_node_basename_doublet_list_by_domain_trail_provider_u_naturalset.ml";; 

(* toplevel 
   #use "Kind_name_constructor_node_basename_doublet_list_by_domain_trail_provider_u_naturalset.ml";;

*)

let nam_dos = "natural";;
let tra_dos = Trail_v.make "domain" nam_dos [];;

let nam_kin_nam_ent_nod_dol = Kind_name_constructor_node_basename_doublet_list_by_domain_trail_provider_v.provide tra_dos;;

test_number 1 (
nam_kin_nam_ent_nod_dol
(* : (string, string) Doublet_t.doublet list *)
=
  [("type", "natural"); ("type", "triangle");
   ("type", "triangle_isoceles"); ("type", "triangle_scalene");
   ("type", "coordinate"); ("type", "coordinate_cartesian");
   ("type", "coordinate_cylindrical"); ("type", "coordinate_spherical");
   ("type", "coordinate_polar")]
);;

