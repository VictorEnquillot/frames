...
let longname sym_dfi =
  Format.sprintf "Camlline_for_tag_datastructure_function_iter_symbol_t.%s" (String.capitalize_ascii (name sym_dfi))
;;
...
let make nam str =
  ...
  with Failure s1 ->
    match s1 with
    | "Not_a_topson_bare:Camlline_for_tag_datastructure_function_iter_symbol_v.ml:make_of_topson_bare" ->
	begin
	  try make_of_topson_ofstring nam str
	  with Failure s2 ->
	    match s2 with
	    | "Not_a_topson_ofstring:Camlline_for_tag_datastructure_function_iter_symbol_v.ml:make_of_topson_ofstring" ->
		begin
		  try make_of_topson_notleaf nam str
		  with Failure s3 ->
		    match s3 with
		    | "Not_a_topson_notleaf:Camlline_for_tag_datastructure_function_iter_symbol_v.ml:make_of_topson_notleaf" ->
			begin
			  let _ =
			  ...
			  in
			  failwith "Not_a_figure_set_fence_point_symbol:Camlline_for_tag_datastructure_function_iter_symbol_v.ml:make"
			end
		    | _ -> failwith s3
		end
	    | _ -> failwith s2
...
(** modified Failures at lundi 29 octobre 2018, 09:35:27 (UTC+0100) *)
