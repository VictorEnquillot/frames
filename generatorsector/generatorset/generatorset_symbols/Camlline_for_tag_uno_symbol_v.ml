...
let longname sym_ftu =
  Format.sprintf "Camlline_for_tag_uno_symbol_t.%s" (String.capitalize_ascii (name sym_ftu))
;;
...
let make_of_topson_notleaf nam s =
  ...
  with Failure s1 ->
    match s1 with
    | "Not_Camlline_for_tag_uno_camltype_symbol:Camlline_for_tag_uno_camltype_symbol_v:make" ->
	begin
	  try camlline_for_tag_uno_symbol_of_camlline_for_tag_uno_top_symbol
	      (Camlline_for_tag_uno_top_symbol_v.make nam s)
	  with Failure s2 ->
	    match s2 with
	    | "Not_Camlline_for_tag_uno_top_symbol:Camlline_for_tag_uno_top_symbol_v:make" ->
...
let make nam str =
  ...
  with Failure s1 ->
    match s1 with
    | "Not_a_topson_bare:Camlline_for_tag_uno_symbol_v_v.ml:make_of_topson_bare" ->
	begin
	  try make_of_topson_ofstring nam str
	  with Failure s2 ->
	    match s2 with
	    | "Not_a_topson_ofstring:Camlline_for_tag_uno_symbol_v_v.ml:make_of_topson_ofstring" ->
		begin
		  try make_of_topson_notleaf nam str
		  with Failure s3 ->
		    match s3 with
		    | "Not_a_topson_notleaf:Camlline_for_tag_uno_symbol_v_v.ml:make_of_topson_notleaf" ->
			begin
			  let _ =
			    Error_messages_v.print_error nam_cod "make"
			      (Format.sprintf "strings \"%s\" and \"%s\" represent an existing Figure_set_fence_point subtype" nam str)
			      "it does not exists"
			      "Check file Camlline_for_tag_uno_symbol_v_v.ml"
			  in
			  failwith "Not_a_figure_set_fence_point_symbol:Camlline_for_tag_uno_symbol_v_v.ml:make"
			end
		    | _ -> failwith s3
		end
	    | _ -> failwith s2
...
(** modified Failures at lundi 29 octobre 2018, 09:22:39 (UTC+0100) *)
