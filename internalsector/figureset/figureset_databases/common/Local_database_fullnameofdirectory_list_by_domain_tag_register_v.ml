...
[
  "Needs: ";
  "Current : DFIG:Local_database_fullnameofdirectory_list_by_domain_tag_register_v";
  "Needed-by: ";
  "What-is-it: ";
  "How-is-it-done: ";
  "Example: ";
...
let nam_reg = "Local_database_fullnameofdirectory_list_by_domain_tag_register_v";;
...
(* let check_is_of_domain_tag_of_son_domain_tag_list domain_tag nam_dip_l = (\*** Improve should be typed **\) *)
...
let store domain_tag nam_dip_l =

  try Register_v.store register domain_tag nam_dip_l
  with Failure ("Already_stored:Register_v.ml:store") ->
    let nam_dip_l_old = retrieve domain_tag in
    let nam_cod = Management_v.current_module_name (documentation ()) in
    if nam_dip_l <> nam_dip_l_old
	...
	(Format.sprintf "for domain_tag >%s<@.    already stored value :@.   %s@.    equal new one :"
	   ...
	   (name_of_value nam_dip_l_old)
	)
	(Format.sprintf "for new one :@.   %s@.Register stores:@.  %s" 
	   (name_of_value nam_dip_l)
	   (dump ())
	)
...
let store_force domain_tag nam_dip_l =
  Register_v.store_force register domain_tag nam_dip_l
;;
