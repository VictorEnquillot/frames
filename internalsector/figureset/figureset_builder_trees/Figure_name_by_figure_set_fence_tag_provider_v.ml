(** {3 Figure_name_by_figure_set_fence_tag_provider_v} *)

(** {6 Documenting} *)

let documentation () = 
  [
   "Current : VFIG:Figure_name_by_figure_set_fence_tag_provider_v";
  ...
  let tag_fig_son =
	...
	(Format.sprintf "String_off List is :@.  %s" 
	   (List_v.name_in_column 
	      (fun t -> (Figure_tag_v.name t) ^ " \"" ^ (Figure_tag_v.string_off t) ^ "\"") 
	      tag_fig_son_l)
	)
	"Check"
  in

  Figure_tag_v.string_off tag_fig_son
;;
...
(* done with do_provider_without_register.sh Figure_name_by_figure_set_fence_tag_provider_v.ml  on mardi 10 janvier 2017, 17:29:32 (UTC+0100) *)
