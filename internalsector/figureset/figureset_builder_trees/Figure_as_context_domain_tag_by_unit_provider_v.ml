...
let documentation () = 
  [
   "Current : BFIG:Figure_as_context_domain_tag_by_unit_provider_v";
   "Needs : BFIG:Basicname_duo_by_fullnameofdirectory_provider_v";
...
let build () =
  let fnd_cur = Unix.getenv ("BFIG") in
  let (bna_dom, bna_sec) = 
  ...
  let sym_coo_fcd = 
    Figure_symbol_v.figure_context_domain_constructor 
      nam_dom 
  in
  Tag_v.make sym_coo_fcd soi_dom
;;
...
(* done with do_provider_without_register.sh Figure_as_context_domain_tag_by_unit_provider_v.ml force on mercredi 12 octobre 2016, 16:11:19 (UTC+0200) *)
