...
testing "Coordinate closure_v with
   Coordinate_closure_u_any.ml"
;;

(* toplevel 
   #use "Coordinate_closure_u_any.ml";; 
...
let tag_poi_l = Figure_field_fence_point_tag_list_by_triangle_tag_provider_v.provide tag_tri;;

test_number 6 (
tag_poi_l
=
  [(Figure_field_fence_point_symbol_t.Figure_field_fence_point_constructor "A", [1; 1; 1; 3]);
   (Figure_field_fence_point_symbol_t.Figure_field_fence_point_constructor "B", [2; 1; 1; 3]);
   (Figure_field_fence_point_symbol_t.Figure_field_fence_point_constructor "C", [3; 1; 1; 3])]
);;

(* 2 Figure_field_fence_point B *)

let nam_poi = "point_constructor";;
let sto_poi = "B";;
let sym_poi = Figure_field_fence_point_symbol_v.make nam_poi sto_poi;;
let tag_poi = Doublet_list_v.find_if_left (fun s -> s = sym_poi) tag_poi_l;;

test_number 7 (
tag_poi 
(* : Figure_field_fence_point_symbol_t.point_symbol Tag_t.tag *)
=
  (Figure_field_fence_point_symbol_t.Figure_field_fence_point_constructor "B", [2; 1; 1; 3])
);;

let tag_coo_l = Figure_field_fence_point_formula_v.coordinate_tag_list_off_point_tag tag_poi;;

let val_coo_l = List.map Coordinate_closure_v.retrieve tag_coo_l;;
