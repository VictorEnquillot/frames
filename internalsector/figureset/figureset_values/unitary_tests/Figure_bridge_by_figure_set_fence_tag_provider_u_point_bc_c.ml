...
(* toplevel 
   #use "Figure_bridge_by_figure_set_fence_tag_provider_u_point_bc_c.ml";; 
...
let tag_fig_bc_c = List_v.only_element_of_predicate_off_list 
    (fun (s, i) -> (Figure_symbol_v.is_figure_set_fence_point_constructor s)
	&& (Figure_symbol_v.string_off s = "BC_C") 
    ) tag_fig_l;;

test_number 1 (
(tag_fig_bc_c : Figure_symbol_t.figure_symbol Tag_t.tag ) =
...
let tag_fsf = Tag_v.map_entity Figure_symbol_v.figure_set_fence_symbol_off_figure_symbol tag_fig_bc_c;;
...
let bri_bc_c = List_v.only_element_of_predicate_off_list 
    (pre_tag_csf_sof sof_csf) 
    tag_coo_l;;

test_number 6 (
(bri_bc_c : Coordinate_symbol_t.coordinate_symbol Tag_t.tag ) =
