...
testing "Parameter_set_body_tuple_envelope_v with
    Parameter_set_body_tuple_envelope_u_try_ao_aopef_hep21.ml";;
...
(* toplevel 
   #use "Parameter_set_body_tuple_envelope_u_try_ao_aopef_hep21.ml";; 

*)

open Parameter_set_body_tuple_envelope_v;;
...
let tag_pbt_hep21 = Parameter_set_body_tuple_tag_v.parameter_set_body_tuple_tag_off_parameter_tag tag_par_hep21;;

(* Argument tag_pbt *)

let tag_pbt = tag_pbt_hep21;;
...
let env_pbt = envelope_of_tag tag_pbt ;;
...
test_number 5 (
make tag_pbt = env_pbt
);;

(** {6 Parameter_envelopee Tuple of Tag} *)

let pee_t = parameter_envelopee_tuple_of_tag tag_pbt;;
...
let sym_psf_t = parameter_set_fence_symbol_tuple_off_envelope env_pbt;;
...
test_number 8 (
parameter_set_fence_symbol_tuple_of_tag tag_pbt = sym_psf_t 
);;

(** {9 Parameter_symbol Tuple} *)

let sym_par_t = parameter_symbol_tuple_off_envelope env_pbt;;
...
test_number 10 (
parameter_symbol_tuple_of_tag tag_pbt = sym_par_t 
);;

(** {6 Readee Tuple of Tag} *)

let rea_t = readee_tuple_of_tag tag_pbt;;
...
test_number 12 (
readee_tuple_of_tag tag_pbt = rea_t
);;

(** {9 Closure off Envelope : tuple of Readee} *)

let clo_pbt = closure_off_envelope env_pbt;;

test_number 13 (
(clo_pbt : Readee_t.readee Tuple_t.tuple ) =
  Tuple_t.Duo
   (Readee_t.Readee_measure
     (Readee_measure_t.Readee_measure
       (2.28187,
...
test_number 14 (
closure_of_tag tag_pbt = clo_pbt
);;

(** {6 Measure Tuple of Tag} *)

let mea_pbt_t = measure_tuple_of_tag tag_pbt;;

test_number 15 (
(mea_pbt_t : Measure_t.measure Tuple_t.tuple ) =
  Tuple_t.Duo
   ((2.28187,
     Units_t.Units_canonical
      (Units_canonical_t.Units_canonical_quantum
...
test_number 16 (
 measure_tuple_of_tag tag_pbt = mea_pbt_t
);;

(** {6 Measure_actual Tuple of Tag} *)

let mac_pbt_t =measure_actual_tuple_of_tag tag_pbt;;
...
let nam_cal = "quantum";;
let mca_pbt_t = measure_canonical_tuple_of_calculation_name_of_tag nam_cal tag_pbt;;
...
let uni_pbt_t = units_tuple_of_tag tag_pbt;;

test_number 19 (
(uni_pbt_t : Units_t.units Tuple_t.tuple ) =
  Tuple_t.Duo
   (Units_t.Units_canonical
     (Units_canonical_t.Units_canonical_quantum
       Units_canonical_quantum_t.Canonical_quantum_length_bohr),
...
test_number 20 ( 
units_tuple_of_tag tag_pbt = uni_pbt_t
);;

(** {9 Float Tuple of Tag} *)

let flo_pbt_t = float_tuple_of_tag tag_pbt;; 

test_number 21 (
(flo_pbt_t : float Tuple_t.tuple) = 
 Tuple_t.Duo (2.28187, 0.231152)
);;

test_number 22 (
float_tuple_of_tag tag_pbt = flo_pbt_t
);;

(** {6 Naming_for_envelope} *)

let nam_env = name env_pbt;;
...
let sof_env = string_off env_pbt;;
