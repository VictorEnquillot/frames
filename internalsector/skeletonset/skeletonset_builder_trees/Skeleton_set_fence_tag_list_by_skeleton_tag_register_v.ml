(** {3 Skeleton_set_fence_tag_list_by_skeleton_tag_register_v} *)

(** {6 Documenting} *)

let documentation () = 
  [
   "Current : BSKE:Skeleton_set_fence_tag_list_by_skeleton_tag_register_v";
   "Author : François Colonna 04 janvier 2017 at 17:44:00+01:00";
 ]
;;
...
let register : 
    (Skeleton_tag_t.skeleton_tag,
     Skeleton_set_fence_tag_t.skeleton_set_fence_tag list
    ) 
    Register_t.register = 
...
let name_of_key key =
  Skeleton_tag_v.name key
;;

let name_of_value val_ =
  List_v.name
    Skeleton_set_fence_tag_v.name
    val_
;;
