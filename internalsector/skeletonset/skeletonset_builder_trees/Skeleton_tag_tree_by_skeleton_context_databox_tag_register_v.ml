(** {3 Skeleton_tag_tree_by_skeleton_context_databox_tag_register_v} *)

(** {6 Documenting} *)

let documentation () = 
  [
   "Current : BSKE:Skeleton_tag_tree_by_skeleton_context_databox_tag_register_v";
...
let register : 
    (
     Skeleton_context_databox_tag_t.skeleton_context_databox_tag,
...
let name_of_key key =
  Skeleton_context_databox_tag_v.fullname
    key
;;

let name_of_value val_ =
  Tree_v.name 
    Skeleton_tag_v.fullname
    val_
;;
...
(* done with do_register.sh force on mardi 11 octobre 2016, 15:02:04 (UTC+0200) *)
