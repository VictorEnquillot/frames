...
let measure_actual_list_off_closure clo =
  let bou_l = readee_list_off_closure clo in
  List.map Readee_v.measure_actual_off_readee bou_l
;;

let measure_canonical_list_of_calculation_name_off_closure nam_cal clo =
  let mea_l = measure_actual_list_off_closure clo in
  List.map (fun m -> 
    Measure_canonical_v.measure_canonical_of_measure_actual_of_calculation_name nam_cal) 
    mea_l
;;
