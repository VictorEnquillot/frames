(** {3 Elementary_tree_symbol_tree_by_nwchem_context_database_tag_provider_v} *)

(** {6 Documenting} *)

let documentation () = 
  [
   "Current : DELE:Elementary_symbol_tree_by_nwchem_context_database_tag_provider_v";
...
let body_symbol_subtree_list tag_dcd =

  let tag_dgn_l = Nwchem_tag_all_list_by_nwchem_context_databox_tag_provider_v.provide tag_dcd in
  let tag_dgn_anv = List.find 
      (fun (s, i) ->
    Nwchem_symbol_v.is_nwchem_body_node_aoset_numerical_values
      )
      tag_dgn_l
  in
  
  let tag_dgn_st = Nwchem_tag_subtree_by_dgn_tag_provider_v.provide tag_dgn_anv
...
let build tag_dcd =
  let sym_bod_sut_l = body_symbol_subtree_list tag_dcd in
  context_symbol_tree sym_bod_sut_l
;;

let build_n_store tag_dcd =
  build tag_dcd 
;;

let provide_without_trace tag_dcd =
  build tag_dcd
;;

let provide_with_trace tag_dcd =
  let nam_mod = Management_v.current_module_name (documentation ()) in
  Management_v.entering_of_current_module_name nam_mod;

  let result = provide_without_trace tag_dcd in
...
let provide tag_dcd =
  ...
  then provide_with_trace tag_dcd
  else provide_without_trace tag_dcd
;;
