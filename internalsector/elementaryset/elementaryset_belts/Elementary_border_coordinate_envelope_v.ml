(** {3 Elementary_border_coordinate_envelope_v} *)

(** {6 Documenting_for_envelope} *)

let documentation () = 
[
 "Current : VELE:Elementary_border_coordinate_envelope_v";
 "Needs: VELE:Elementary_border_coordinate_belt_v";
 "Needed-by: ";
...
let elementary_border_tag_of_tag tag_ebc =
  Tag_v.map_entity 
...
let envelopee_of_tag tag_ebc =
  let tag_ebo = elementary_border_tag_of_tag tag_ebc in
...
let envelope_of_tag tag_ebc =
  let env= envelopee_of_tag tag_ebc in
  [env]
;;
...
let tagged_measure_actual_of_tag tag_ebc =
  let mea_ebc = measure_actual_of_tag tag_ebc in 
  Doublet_v.make tag_ebc mea_ebc
;;
...
let envelopee_off_envelope env_ebc =
  try List_v.element_off_one_element_list env_ebc 
  with Failure _ -> 
    Error_messages_v.print_fatal_error __LOC__ "envelopee_off_envelope"
      "One element list"
      (Format.sprintf "Envelopee list is :@.   %s" (List_v.name Envelopee_v.fullname env_ebc))
      "Check"
;;

(** {6 Elementary_border_tag off Envelope} *)

let elementary_border_tag_off_envelope env_ebc =
  let env = envelopee_off_envelope env_ebc in
  Doublet_v.left_off_doublet env
;;

(** {6 Tagged_readee} *)

let tagged_readee_of_tag tag_ebc =
  let rea_ebc = readee_of_tag tag_ebc in
  Doublet_v.make tag_ebc rea_ebc
;;
...
let readee_off_envelope env_ebc =
  let env = envelopee_off_envelope env_ebc in
...
let measure_actual_off_envelope env_ebc =
  let env = envelopee_off_envelope env_ebc in
  Envelopee_v.measure_actual_off_envelopee env
;;

let measure_actual_list_off_envelope env_ebc =
  let mea = measure_actual_off_envelope env_ebc in
  [mea]
;;

let measure_canonical_off_envelope env_ebc =
  let mea = measure_actual_off_envelope env_ebc in
  Measure_actual_v.measure_canonical_of_measure_actual_of_calculation_name mea
;;

let measure_canonical_list_off_envelope env_ebc =
  let can_mea = measure_canonical_off_envelope env_ebc in
  [can_mea]
;;

(** {6 Closure : list of Readee} *)

let closure_off_envelope env_ebc =
  let rea = readee_off_envelope env_ebc in
  [rea]
;;
...
let name env_ebc =
  List_v.name Envelopee_v.name env_ebc
;;

let fullname env_ebc =
  List_v.name Envelopee_v.fullname env_ebc
;;

let longname env_ebc =
  List_v.name Envelopee_v.longname env_ebc
;;

(* generated by ./do_envelope_as_list_border_trunc_units_v.sh elementary_border_coordinate ebc elementary_fence_units_actual *)
