(** {3 Elementary_border_parameter_ecppef_envelope_v} *)

(** {6 Documenting_for_envelope} *)

let documentation () = 
[
 "Current : VELE:Elementary_border_parameter_ecppef_envelope_v";
 "Needs: VELE:Elementary_border_parameter_ecppef_belt_v";
 "Needed-by: ";
...
let elementary_border_tag_of_tag tag_epe =
  Tag_v.map_entity 
...
let envelopee_of_tag tag_epe =
  let tag_ebo = elementary_border_tag_of_tag tag_epe in
...
let envelope_of_tag tag_epe =
  let env= envelopee_of_tag tag_epe in
  [env]
;;
...
let measure_actual_of_tag tag_epe =
  Elementary_border_parameter_ecppef_belt_v.measure_actual_of_tag tag_epe
;;
...
let tagged_measure_actual_of_tag tag_epe =
  let mea_epe = measure_actual_of_tag tag_epe in 
  Doublet_v.make tag_epe mea_epe
;;
...
let envelopee_off_envelope env_epe =
  try List_v.element_off_one_element_list env_epe 
  with Failure _ -> 
    Error_messages_v.print_fatal_error __LOC__ "envelopee_off_envelope"
      "One element list"
      (Format.sprintf "Envelopee list is :@.   %s" (List_v.name Envelopee_v.fullname env_epe))
      "Check"
;;

(** {6 Elementary_border_tag off Envelope} *)

let elementary_border_tag_off_envelope env_epe =
  let env = envelopee_off_envelope env_epe in
  Doublet_v.left_off_doublet env
;;

(** {6 Tagged_readee} *)

let tagged_readee_of_tag tag_epe =
  let rea_epe = readee_of_tag tag_epe in
  Doublet_v.make tag_epe rea_epe
;;
...
let readee_off_envelope env_epe =
  let env = envelopee_off_envelope env_epe in
...
let measure_actual_off_envelope env_epe =
  let env = envelopee_off_envelope env_epe in
  Envelopee_v.measure_actual_off_envelopee env
;;

let measure_actual_list_off_envelope env_epe =
  let mea = measure_actual_off_envelope env_epe in
  [mea]
;;

let measure_canonical_off_envelope env_epe =
  let mea = measure_actual_off_envelope env_epe in
  Measure_actual_v.measure_canonical_of_measure_actual_of_calculation_name mea
;;

let measure_canonical_list_off_envelope env_epe =
  let can_mea = measure_canonical_off_envelope env_epe in
  [can_mea]
;;

(** {6 Closure : list of Readee} *)

let closure_off_envelope env_epe =
  let rea = readee_off_envelope env_epe in
  [rea]
;;
...
let name env_epe =
  List_v.name Envelopee_v.name env_epe
;;

let fullname env_epe =
  List_v.name Envelopee_v.fullname env_epe
;;

let longname env_epe =
  List_v.name Envelopee_v.longname env_epe
;;

(* generated by ./do_envelope_as_list_border_trunc_units_v.sh elementary_border_parameter_ecppef epe elementary_fence_units_actual *)
