(** {3 Elementary_border_parameter_aopef_envelope_v} *)

(** {6 Documenting_for_envelope} *)

let documentation () = 
[
 "Current : VELE:Elementary_border_parameter_aopef_envelope_v";
 "Needs: VELE:Elementary_border_parameter_aopef_belt_v";
 "Needed-by: ";
...
let elementary_border_tag_of_tag tag_epa =
  Tag_v.map_entity 
...
let envelopee_of_tag tag_epa =
  let tag_ebo = elementary_border_tag_of_tag tag_epa in
...
let envelope_of_tag tag_epa =
  let env= envelopee_of_tag tag_epa in
  [env]
;;
...
let measure_actual_of_tag tag_epa =
  Elementary_border_parameter_aopef_belt_v.measure_actual_of_tag tag_epa
;;
...
let tagged_measure_actual_of_tag tag_epa =
  let mea_epa = measure_actual_of_tag tag_epa in 
  Doublet_v.make tag_epa mea_epa
;;
...
let envelopee_off_envelope env_epa =
  try List_v.element_off_one_element_list env_epa 
  with Failure _ -> 
    Error_messages_v.print_fatal_error __LOC__ "envelopee_off_envelope"
      "One element list"
      (Format.sprintf "Envelopee list is :@.   %s" (List_v.name Envelopee_v.fullname env_epa))
      "Check"
;;

(** {6 Elementary_border_tag off Envelope} *)

let elementary_border_tag_off_envelope env_epa =
  let env = envelopee_off_envelope env_epa in
  Doublet_v.left_off_doublet env
;;

(** {6 Tagged_readee} *)

let tagged_readee_of_tag tag_epa =
  let rea_epa = readee_of_tag tag_epa in
  Doublet_v.make tag_epa rea_epa
;;
...
let readee_off_envelope env_epa =
  let env = envelopee_off_envelope env_epa in
...
let measure_actual_off_envelope env_epa =
  let env = envelopee_off_envelope env_epa in
  Envelopee_v.measure_actual_off_envelopee env
;;

let measure_actual_list_off_envelope env_epa =
  let mea = measure_actual_off_envelope env_epa in
  [mea]
;;

let measure_canonical_off_envelope env_epa =
  let mea = measure_actual_off_envelope env_epa in
  Measure_actual_v.measure_canonical_of_measure_actual_of_calculation_name mea
;;

let measure_canonical_list_off_envelope env_epa =
  let can_mea = measure_canonical_off_envelope env_epa in
  [can_mea]
;;

(** {6 Closure : list of Readee} *)

let closure_off_envelope env_epa =
  let rea = readee_off_envelope env_epa in
  [rea]
;;
...
let name env_epa =
  List_v.name Envelopee_v.name env_epa
;;

let fullname env_epa =
  List_v.name Envelopee_v.fullname env_epa
;;

let longname env_epa =
  List_v.name Envelopee_v.longname env_epa
;;

(* generated by ./do_envelope_as_list_border_trunc_units_v.sh elementary_border_parameter_aopef epa elementary_fence_units_actual *)
