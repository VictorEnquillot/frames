(** {3 Elementary_border_parameter_envelope_v} *)

(** {6 Documenting_for_envelope} *)

let documentation () = 
[
 "Current : VELE:Elementary_border_parameter_envelope_v";
 "Needs: VELE:Elementary_border_parameter_belt_v";
 "Needed-by: ";
...
let elementary_border_tag_of_tag tag_ebp =
  Tag_v.map_entity 
...
let envelopee_of_tag tag_ebp =
  let tag_ebo = elementary_border_tag_of_tag tag_ebp in
...
let envelope_of_tag tag_ebp =
  let env= envelopee_of_tag tag_ebp in
  [env]
;;
...
let tagged_measure_actual_of_tag tag_ebp =
  let mea_ebp = measure_actual_of_tag tag_ebp in 
  Doublet_v.make tag_ebp mea_ebp
;;
...
let envelopee_off_envelope env_ebp =
  try List_v.element_off_one_element_list env_ebp 
  with Failure _ -> 
    Error_messages_v.print_fatal_error __LOC__ "envelopee_off_envelope"
      "One element list"
      (Format.sprintf "Envelopee list is :@.   %s" (List_v.name Envelopee_v.fullname env_ebp))
      "Check"
;;

(** {6 Elementary_border_tag off Envelope} *)

let elementary_border_tag_off_envelope env_ebp =
  let env = envelopee_off_envelope env_ebp in
  Doublet_v.left_off_doublet env
;;

(** {6 Tagged_readee} *)

let tagged_readee_of_tag tag_ebp =
  let rea_ebp = readee_of_tag tag_ebp in
  Doublet_v.make tag_ebp rea_ebp
;;
...
let readee_off_envelope env_ebp =
  let env = envelopee_off_envelope env_ebp in
...
let measure_actual_off_envelope env_ebp =
  let env = envelopee_off_envelope env_ebp in
  Envelopee_v.measure_actual_off_envelopee env
;;

let measure_actual_list_off_envelope env_ebp =
  let mea = measure_actual_off_envelope env_ebp in
  [mea]
;;

let measure_canonical_off_envelope env_ebp =
  let mea = measure_actual_off_envelope env_ebp in
  Measure_actual_v.measure_canonical_of_measure_actual_of_calculation_name mea
;;

let measure_canonical_list_off_envelope env_ebp =
  let can_mea = measure_canonical_off_envelope env_ebp in
  [can_mea]
;;

(** {6 Closure : list of Readee} *)

let closure_off_envelope env_ebp =
  let rea = readee_off_envelope env_ebp in
  [rea]
;;
...
let name env_ebp =
  List_v.name Envelopee_v.name env_ebp
;;

let fullname env_ebp =
  List_v.name Envelopee_v.fullname env_ebp
;;

let longname env_ebp =
  List_v.name Envelopee_v.longname env_ebp
;;

(* generated by ./do_envelope_as_list_border_trunc_units_v.sh elementary_border_parameter ebp elementary_fence_units_actual *)
