(** {3 Elementary_border_coordinate_homogeneous_planar_envelope_v} *)

(** {6 Documenting_for_envelope} *)

let documentation () = 
[
 "Current : VELE:Elementary_border_coordinate_homogeneous_planar_envelope_v";
 "Needs: VELE:Elementary_border_coordinate_homogeneous_planar_belt_v";
 "Needed-by: ";
...
let elementary_border_tag_of_tag tag_ehp =
  Tag_v.map_entity 
...
let envelopee_of_tag tag_ehp =
  let tag_ebo = elementary_border_tag_of_tag tag_ehp in
...
let envelope_of_tag tag_ehp =
  let env= envelopee_of_tag tag_ehp in
  [env]
;;
...
let measure_actual_of_tag tag_ehp =
  Elementary_border_coordinate_homogeneous_planar_belt_v.measure_actual_of_tag tag_ehp
;;
...
let tagged_measure_actual_of_tag tag_ehp =
  let mea_ehp = measure_actual_of_tag tag_ehp in 
  Doublet_v.make tag_ehp mea_ehp
;;
...
let envelopee_off_envelope env_ehp =
  try List_v.element_off_one_element_list env_ehp 
  with Failure _ -> 
    Error_messages_v.print_fatal_error __LOC__ "envelopee_off_envelope"
      "One element list"
      (Format.sprintf "Envelopee list is :@.   %s" (List_v.name Envelopee_v.fullname env_ehp))
      "Check"
;;

(** {6 Elementary_border_tag off Envelope} *)

let elementary_border_tag_off_envelope env_ehp =
  let env = envelopee_off_envelope env_ehp in
  Doublet_v.left_off_doublet env
;;

(** {6 Tagged_readee} *)

let tagged_readee_of_tag tag_ehp =
  let rea_ehp = readee_of_tag tag_ehp in
  Doublet_v.make tag_ehp rea_ehp
;;
...
let readee_off_envelope env_ehp =
  let env = envelopee_off_envelope env_ehp in
...
let measure_actual_off_envelope env_ehp =
  let env = envelopee_off_envelope env_ehp in
  Envelopee_v.measure_actual_off_envelopee env
;;

let measure_actual_list_off_envelope env_ehp =
  let mea = measure_actual_off_envelope env_ehp in
  [mea]
;;

let measure_canonical_off_envelope env_ehp =
  let mea = measure_actual_off_envelope env_ehp in
  Measure_actual_v.measure_canonical_of_measure_actual_of_calculation_name mea
;;

let measure_canonical_list_off_envelope env_ehp =
  let can_mea = measure_canonical_off_envelope env_ehp in
  [can_mea]
;;

(** {6 Closure : list of Readee} *)

let closure_off_envelope env_ehp =
  let rea = readee_off_envelope env_ehp in
  [rea]
;;
...
let name env_ehp =
  List_v.name Envelopee_v.name env_ehp
;;

let fullname env_ehp =
  List_v.name Envelopee_v.fullname env_ehp
;;

let longname env_ehp =
  List_v.name Envelopee_v.longname env_ehp
;;

(* generated by ./do_envelope_as_list_border_trunc_units_v.sh elementary_border_coordinate_homogeneous_planar ehp elementary_fence_units_actual *)
