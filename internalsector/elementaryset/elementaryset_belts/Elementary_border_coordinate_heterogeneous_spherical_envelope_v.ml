(** {3 Elementary_border_coordinate_heterogeneous_spherical_envelope_v} *)

(** {6 Documenting_for_envelope} *)

let documentation () = 
[
 "Current : VELE:Elementary_border_coordinate_heterogeneous_spherical_envelope_v";
 "Needs: VELE:Elementary_border_coordinate_heterogeneous_spherical_belt_v";
 "Needed-by: ";
...
let elementary_border_tag_of_tag tag_ehs =
  Tag_v.map_entity 
...
let envelopee_of_tag tag_ehs =
  let tag_ebo = elementary_border_tag_of_tag tag_ehs in
...
let envelope_of_tag tag_ehs =
  let env= envelopee_of_tag tag_ehs in
  [env]
;;
...
let tagged_measure_actual_of_tag tag_ehs =
  let mea_ehs = measure_actual_of_tag tag_ehs in 
  Doublet_v.make tag_ehs mea_ehs
;;
...
let envelopee_off_envelope env_ehs =
  try List_v.element_off_one_element_list env_ehs 
  with Failure _ -> 
    Error_messages_v.print_fatal_error __LOC__ "envelopee_off_envelope"
      "One element list"
      (Format.sprintf "Envelopee list is :@.   %s" (List_v.name Envelopee_v.fullname env_ehs))
      "Check"
;;

(** {6 Elementary_border_tag off Envelope} *)

let elementary_border_tag_off_envelope env_ehs =
  let env = envelopee_off_envelope env_ehs in
  Doublet_v.left_off_doublet env
;;

(** {6 Tagged_readee} *)

let tagged_readee_of_tag tag_ehs =
  let rea_ehs = readee_of_tag tag_ehs in
  Doublet_v.make tag_ehs rea_ehs
;;
...
let readee_off_envelope env_ehs =
  let env = envelopee_off_envelope env_ehs in
...
let measure_actual_off_envelope env_ehs =
  let env = envelopee_off_envelope env_ehs in
  Envelopee_v.measure_actual_off_envelopee env
;;

let measure_actual_list_off_envelope env_ehs =
  let mea = measure_actual_off_envelope env_ehs in
  [mea]
;;

let measure_canonical_off_envelope env_ehs =
  let mea = measure_actual_off_envelope env_ehs in
  Measure_actual_v.measure_canonical_of_measure_actual_of_calculation_name mea
;;

let measure_canonical_list_off_envelope env_ehs =
  let can_mea = measure_canonical_off_envelope env_ehs in
  [can_mea]
;;

(** {6 Closure : list of Readee} *)

let closure_off_envelope env_ehs =
  let rea = readee_off_envelope env_ehs in
  [rea]
;;
...
let name env_ehs =
  List_v.name Envelopee_v.name env_ehs
;;

let fullname env_ehs =
  List_v.name Envelopee_v.fullname env_ehs
;;

let longname env_ehs =
  List_v.name Envelopee_v.longname env_ehs
;;

(* generated by ./do_envelope_as_list_border_trunc_units_v.sh elementary_border_coordinate_heterogeneous_spherical ehs elementary_fence_units_actual *)
