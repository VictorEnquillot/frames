(** {3 Elementary_border_coordinate_homogeneous_linear_envelope_v} *)

(** {6 Documenting_for_envelope} *)

let documentation () = 
[
 "Current : VELE:Elementary_border_coordinate_homogeneous_linear_envelope_v";
 "Needs: VELE:Elementary_border_coordinate_homogeneous_linear_belt_v";
 "Needed-by: ";
...
let elementary_border_tag_of_tag tag_ehl =
  Tag_v.map_entity 
...
let envelopee_of_tag tag_ehl =
  let tag_ebo = elementary_border_tag_of_tag tag_ehl in
...
let envelope_of_tag tag_ehl =
  let env= envelopee_of_tag tag_ehl in
  [env]
;;
...
let measure_actual_of_tag tag_ehl =
  Elementary_border_coordinate_homogeneous_linear_belt_v.measure_actual_of_tag tag_ehl
;;
...
let tagged_measure_actual_of_tag tag_ehl =
  let mea_ehl = measure_actual_of_tag tag_ehl in 
  Doublet_v.make tag_ehl mea_ehl
;;
...
let envelopee_off_envelope env_ehl =
  try List_v.element_off_one_element_list env_ehl 
  with Failure _ -> 
    Error_messages_v.print_fatal_error __LOC__ "envelopee_off_envelope"
      "One element list"
      (Format.sprintf "Envelopee list is :@.   %s" (List_v.name Envelopee_v.fullname env_ehl))
      "Check"
;;

(** {6 Elementary_border_tag off Envelope} *)

let elementary_border_tag_off_envelope env_ehl =
  let env = envelopee_off_envelope env_ehl in
  Doublet_v.left_off_doublet env
;;

(** {6 Tagged_readee} *)

let tagged_readee_of_tag tag_ehl =
  let rea_ehl = readee_of_tag tag_ehl in
  Doublet_v.make tag_ehl rea_ehl
;;
...
let readee_off_envelope env_ehl =
  let env = envelopee_off_envelope env_ehl in
...
let measure_actual_off_envelope env_ehl =
  let env = envelopee_off_envelope env_ehl in
  Envelopee_v.measure_actual_off_envelopee env
;;

let measure_actual_list_off_envelope env_ehl =
  let mea = measure_actual_off_envelope env_ehl in
  [mea]
;;

let measure_canonical_off_envelope env_ehl =
  let mea = measure_actual_off_envelope env_ehl in
  Measure_actual_v.measure_canonical_of_measure_actual_of_calculation_name mea
;;

let measure_canonical_list_off_envelope env_ehl =
  let can_mea = measure_canonical_off_envelope env_ehl in
  [can_mea]
;;

(** {6 Closure : list of Readee} *)

let closure_off_envelope env_ehl =
  let rea = readee_off_envelope env_ehl in
  [rea]
;;
...
let name env_ehl =
  List_v.name Envelopee_v.name env_ehl
;;

let fullname env_ehl =
  List_v.name Envelopee_v.fullname env_ehl
;;

let longname env_ehl =
  List_v.name Envelopee_v.longname env_ehl
;;

(* generated by ./do_envelope_as_list_border_trunc_units_v.sh elementary_border_coordinate_homogeneous_linear ehl elementary_fence_units_actual *)
