(** {3 Elementary_border_coordinate_homogeneous_planar_y_container_v} *)

(** {6 Documenting_for_container} *)

let documentation () = 
  [
   "Current : FELE:Elementary_border_coordinate_homogeneous_planar_y_container_v";
...
let string_off con_epy =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.string_off
    con_epy
;;

let longname con_epy =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.longname
    con_epy
;;

let fullname con_epy =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.fullname
    con_epy
;;

(** {6 Coercing Up Elementary_border_coordinate_homogeneous_planar_y Tag} *)

let coerce_up_tag tag_epy =
  Tag_v.map_entity 
    Elementary_symbol_v.elementary_symbol_of_elementary_border_coordinate_homogeneous_planar_y_symbol 
    tag_epy
;;

(** {6 Builder Tag list} *)

let builder_tag_list tag_epy =
  let tag_ele = coerce_up_tag tag_epy in
  Son_elementary_tag_list_by_father_elementary_tag_provider_v.provide tag_ele
;;

(** {6 Builder Tag Datastructure} *)

let builder_tag_uno tag_epy =
  let tag_bui_l = builder_tag_list tag_epy in
  Uno_v.uno_of_list tag_bui_l
;;

(** {6 Building as Builder Tag uno} *)

let building_of_tag tag_epy =
  builder_tag_uno tag_epy
;;

(** {6 Builder Tag tuple} *)

let builder_tag_tuple tag_epy =
  let tag_bui_u = builder_tag_uno tag_epy in
...
let container_of_tag tag_epy =
  let bug = building_of_tag tag_epy in
...
let containee_tag_uno tag_epy =
  container_of_tag tag_epy
;;

let containee_tag_tuple tag_epy =
  let tag_cne_u = containee_tag_uno tag_epy in
  Tuple_v.tuple_of_uno tag_cne_u
;;

(** {6 Border *)

let border_as_list tag_epy =
  let tag_ele = coerce_up_tag tag_epy in
  Elementary_border_by_elementary_tag_provider_v.provide tag_ele
;;

let border_as_duo tag_epy =
  let bor_epy = border_as_list tag_epy in
  Duo_v.duo_of_list bor_epy
;;

let border_as_tuple tag_epy =
  let bor_l = border_as_list tag_epy in
  Tuple_v.tuple_of_list bor_l
;;

let border_as_tuple_list tag_epy =
  let bor_l = border_as_list tag_epy in
...
let fence_as_list tag_epy =
  let tag_ele = coerce_up_tag tag_epy in
  Elementary_fence_by_elementary_tag_provider_v.provide tag_ele
;;

let fence_as_uno tag_epy =
  let fen_epy = fence_as_list tag_epy in
  Uno_v.uno_of_list fen_epy
;;

let fence_as_tuple tag_epy =
  let fen_l = fence_as_list tag_epy in
  Tuple_v.tuple_of_list fen_l
;;

let fence_as_tuple_list tag_epy =
  let fen_l = fence_as_list tag_epy in
  Tuple_list_v.make fen_l
;;


(* generated by ./do_container_as_uno_border_bud_1containee_v.sh elementary_border_coordinate_homogeneous_planar_y epy elementary_fence_units_actual_length eal *)
