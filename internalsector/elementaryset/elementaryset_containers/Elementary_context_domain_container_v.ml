(** {3 Elementary_context_domain_container_v} *)

(** {6 Documenting_for_container} *)

let documentation () = 
  [
   "Current : FELE:Elementary_context_domain_container_v";
   "Needs : FELE:Elementary_context_database_tag_v";
   ...
   "What-is-it : it is a Bud container of Context sub-Types and thus a List of its Elementary_context_database Tag";
 ]
;;
...
let name con_ecd =
  List_v.name 
    Elementary_context_database_tag_v.name
    con_ecd
;;

let string_off con_ecd =
  List_v.name 
    Elementary_context_database_tag_v.string_off
    con_ecd
;;

let longname con_ecd =
  List_v.name 
    Elementary_context_database_tag_v.longname
    con_ecd
;;

let fullname con_ecd =
  List_v.name 
    Elementary_context_database_tag_v.fullname
    con_ecd
;;
...
let builder_tag_list tag_ecd =
  let tag_ele = coerce_up_tag tag_ecd in
...
let builder_tag_tuple tag_ecd =
  let tag_bui_l = builder_tag_list tag_ecd in
  Tuple_v.tuple_of_list tag_bui_l
;;

(** {9 Building as Builder Tag tuple} *)

let building_of_tag tag_ecd =
  builder_tag_list tag_ecd
;;

(** {6 Container} *)

let containee_up_tag_list tag_ecd =
  let tag_bui_l = builder_tag_list tag_ecd in
...
let border_as_list tag_ecd =
  let tag_ele = coerce_up_tag tag_ecd in
  Elementary_border_by_elementary_tag_provider_v.provide tag_ele
;;

let border_as_duo tag_ecd =
  let bor_ecd = border_as_list tag_ecd in
  Duo_v.duo_of_list bor_ecd
;;

let border_as_tuple tag_ecd =
  let bor_l = border_as_list tag_ecd in
  Tuple_v.tuple_of_list bor_l
;;

let border_as_tuple_list tag_ecd =
  let bor_l = border_as_list tag_ecd in
...
let fence_as_list tag_ecd =
  let tag_ele = coerce_up_tag tag_ecd in
  Elementary_fence_by_elementary_tag_provider_v.provide tag_ele
;;

let fence_as_tuple tag_ecd =
  let fen_l = fence_as_list tag_ecd in
  Tuple_v.tuple_of_list fen_l
;;

let fence_as_tuple_list tag_ecd =
  let fen_l = fence_as_list tag_ecd in
  Tuple_list_v.make fen_l
;;

(* generated by ./do_container_as_list_context_bud_1containee_v.sh elementary_context_domain ecd elementary_context_database *)
