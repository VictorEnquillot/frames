(** {3 Elementary_border_coordinate_heterogeneous_cylindrical_rho_container_v} *)

(** {6 Documenting_for_container} *)

let documentation () = 
  [
   "Current : FELE:Elementary_border_coordinate_heterogeneous_cylindrical_rho_container_v";
...
let string_off con_ecr =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.string_off
    con_ecr
;;

let longname con_ecr =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.longname
    con_ecr
;;

let fullname con_ecr =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.fullname
    con_ecr
;;

(** {6 Coercing Up Elementary_border_coordinate_heterogeneous_cylindrical_rho Tag} *)

let coerce_up_tag tag_ecr =
  Tag_v.map_entity 
    Elementary_symbol_v.elementary_symbol_of_elementary_border_coordinate_heterogeneous_cylindrical_rho_symbol 
    tag_ecr
;;

(** {6 Builder Tag list} *)

let builder_tag_list tag_ecr =
  let tag_ele = coerce_up_tag tag_ecr in
  Son_elementary_tag_list_by_father_elementary_tag_provider_v.provide tag_ele
;;

(** {6 Builder Tag Datastructure} *)

let builder_tag_uno tag_ecr =
  let tag_bui_l = builder_tag_list tag_ecr in
  Uno_v.uno_of_list tag_bui_l
;;

(** {6 Building as Builder Tag uno} *)

let building_of_tag tag_ecr =
  builder_tag_uno tag_ecr
;;

(** {6 Builder Tag tuple} *)

let builder_tag_tuple tag_ecr =
  let tag_bui_u = builder_tag_uno tag_ecr in
...
let container_of_tag tag_ecr =
  let bug = building_of_tag tag_ecr in
...
let containee_tag_uno tag_ecr =
  container_of_tag tag_ecr
;;

let containee_tag_tuple tag_ecr =
  let tag_cne_u = containee_tag_uno tag_ecr in
  Tuple_v.tuple_of_uno tag_cne_u
;;

(** {6 Border *)

let border_as_list tag_ecr =
  let tag_ele = coerce_up_tag tag_ecr in
  Elementary_border_by_elementary_tag_provider_v.provide tag_ele
;;

let border_as_duo tag_ecr =
  let bor_ecr = border_as_list tag_ecr in
  Duo_v.duo_of_list bor_ecr
;;

let border_as_tuple tag_ecr =
  let bor_l = border_as_list tag_ecr in
  Tuple_v.tuple_of_list bor_l
;;

let border_as_tuple_list tag_ecr =
  let bor_l = border_as_list tag_ecr in
...
let fence_as_list tag_ecr =
  let tag_ele = coerce_up_tag tag_ecr in
  Elementary_fence_by_elementary_tag_provider_v.provide tag_ele
;;

let fence_as_uno tag_ecr =
  let fen_ecr = fence_as_list tag_ecr in
  Uno_v.uno_of_list fen_ecr
;;

let fence_as_tuple tag_ecr =
  let fen_l = fence_as_list tag_ecr in
  Tuple_v.tuple_of_list fen_l
;;

let fence_as_tuple_list tag_ecr =
  let fen_l = fence_as_list tag_ecr in
  Tuple_list_v.make fen_l
;;


(* generated by ./do_container_as_uno_border_bud_1containee_v.sh elementary_border_coordinate_heterogeneous_cylindrical_rho ecr elementary_fence_units_actual_length eal *)
