(** {3 Elementary_border_parameter_ecppef_coefficient_container_v} *)

(** {6 Documenting_for_container} *)

let documentation () = 
  [
   "Current : FELE:Elementary_border_parameter_ecppef_coefficient_container_v";
...
let string_off con_eec =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.string_off
    con_eec
;;

let longname con_eec =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.longname
    con_eec
;;

let fullname con_eec =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.fullname
    con_eec
;;

(** {6 Coercing Up Elementary_border_parameter_ecppef_coefficient Tag} *)

let coerce_up_tag tag_eec =
  Tag_v.map_entity 
    Elementary_symbol_v.elementary_symbol_of_elementary_border_parameter_ecppef_coefficient_symbol 
    tag_eec
;;

(** {6 Builder Tag list} *)

let builder_tag_list tag_eec =
  let tag_ele = coerce_up_tag tag_eec in
  Son_elementary_tag_list_by_father_elementary_tag_provider_v.provide tag_ele
;;

(** {6 Builder Tag Datastructure} *)

let builder_tag_uno tag_eec =
  let tag_bui_l = builder_tag_list tag_eec in
  Uno_v.uno_of_list tag_bui_l
;;

(** {6 Building as Builder Tag uno} *)

let building_of_tag tag_eec =
  builder_tag_uno tag_eec
;;

(** {6 Builder Tag tuple} *)

let builder_tag_tuple tag_eec =
  let tag_bui_u = builder_tag_uno tag_eec in
...
let container_of_tag tag_eec =
  let bug = building_of_tag tag_eec in
...
let containee_tag_uno tag_eec =
  container_of_tag tag_eec
;;

let containee_tag_tuple tag_eec =
  let tag_cne_u = containee_tag_uno tag_eec in
  Tuple_v.tuple_of_uno tag_cne_u
;;

(** {6 Border *)

let border_as_list tag_eec =
  let tag_ele = coerce_up_tag tag_eec in
  Elementary_border_by_elementary_tag_provider_v.provide tag_ele
;;

let border_as_duo tag_eec =
  let bor_eec = border_as_list tag_eec in
  Duo_v.duo_of_list bor_eec
;;

let border_as_tuple tag_eec =
  let bor_l = border_as_list tag_eec in
  Tuple_v.tuple_of_list bor_l
;;

let border_as_tuple_list tag_eec =
  let bor_l = border_as_list tag_eec in
...
let fence_as_list tag_eec =
  let tag_ele = coerce_up_tag tag_eec in
  Elementary_fence_by_elementary_tag_provider_v.provide tag_ele
;;

let fence_as_uno tag_eec =
  let fen_eec = fence_as_list tag_eec in
  Uno_v.uno_of_list fen_eec
;;

let fence_as_tuple tag_eec =
  let fen_l = fence_as_list tag_eec in
  Tuple_v.tuple_of_list fen_l
;;

let fence_as_tuple_list tag_eec =
  let fen_l = fence_as_list tag_eec in
  Tuple_list_v.make fen_l
;;


(* generated by ./do_container_as_uno_border_bud_1containee_v.sh elementary_border_parameter_ecppef_coefficient eec elementary_fence_units_actual_length eal *)
