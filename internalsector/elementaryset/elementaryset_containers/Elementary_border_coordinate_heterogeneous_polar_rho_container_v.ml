(** {3 Elementary_border_coordinate_heterogeneous_polar_rho_container_v} *)

(** {6 Documenting_for_container} *)

let documentation () = 
  [
   "Current : FELE:Elementary_border_coordinate_heterogeneous_polar_rho_container_v";
...
let string_off con_epr =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.string_off
    con_epr
;;

let longname con_epr =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.longname
    con_epr
;;

let fullname con_epr =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.fullname
    con_epr
;;

(** {6 Coercing Up Elementary_border_coordinate_heterogeneous_polar_rho Tag} *)

let coerce_up_tag tag_epr =
  Tag_v.map_entity 
    Elementary_symbol_v.elementary_symbol_of_elementary_border_coordinate_heterogeneous_polar_rho_symbol 
    tag_epr
;;

(** {6 Builder Tag list} *)

let builder_tag_list tag_epr =
  let tag_ele = coerce_up_tag tag_epr in
  Son_elementary_tag_list_by_father_elementary_tag_provider_v.provide tag_ele
;;

(** {6 Builder Tag Datastructure} *)

let builder_tag_uno tag_epr =
  let tag_bui_l = builder_tag_list tag_epr in
  Uno_v.uno_of_list tag_bui_l
;;

(** {6 Building as Builder Tag uno} *)

let building_of_tag tag_epr =
  builder_tag_uno tag_epr
;;

(** {6 Builder Tag tuple} *)

let builder_tag_tuple tag_epr =
  let tag_bui_u = builder_tag_uno tag_epr in
...
let container_of_tag tag_epr =
  let bug = building_of_tag tag_epr in
...
let containee_tag_uno tag_epr =
  container_of_tag tag_epr
;;

let containee_tag_tuple tag_epr =
  let tag_cne_u = containee_tag_uno tag_epr in
  Tuple_v.tuple_of_uno tag_cne_u
;;

(** {6 Border *)

let border_as_list tag_epr =
  let tag_ele = coerce_up_tag tag_epr in
  Elementary_border_by_elementary_tag_provider_v.provide tag_ele
;;

let border_as_duo tag_epr =
  let bor_epr = border_as_list tag_epr in
  Duo_v.duo_of_list bor_epr
;;

let border_as_tuple tag_epr =
  let bor_l = border_as_list tag_epr in
  Tuple_v.tuple_of_list bor_l
;;

let border_as_tuple_list tag_epr =
  let bor_l = border_as_list tag_epr in
...
let fence_as_list tag_epr =
  let tag_ele = coerce_up_tag tag_epr in
  Elementary_fence_by_elementary_tag_provider_v.provide tag_ele
;;

let fence_as_uno tag_epr =
  let fen_epr = fence_as_list tag_epr in
  Uno_v.uno_of_list fen_epr
;;

let fence_as_tuple tag_epr =
  let fen_l = fence_as_list tag_epr in
  Tuple_v.tuple_of_list fen_l
;;

let fence_as_tuple_list tag_epr =
  let fen_l = fence_as_list tag_epr in
  Tuple_list_v.make fen_l
;;


(* generated by ./do_container_as_uno_border_bud_1containee_v.sh elementary_border_coordinate_heterogeneous_polar_rho epr elementary_fence_units_actual_length eal *)
