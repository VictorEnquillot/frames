(** {3 Elementary_border_coordinate_heterogeneous_spherical_rho_container_v} *)

(** {6 Documenting_for_container} *)

let documentation () = 
  [
   "Current : FELE:Elementary_border_coordinate_heterogeneous_spherical_rho_container_v";
...
let string_off con_esr =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.string_off
    con_esr
;;

let longname con_esr =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.longname
    con_esr
;;

let fullname con_esr =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.fullname
    con_esr
;;

(** {6 Coercing Up Elementary_border_coordinate_heterogeneous_spherical_rho Tag} *)

let coerce_up_tag tag_esr =
  Tag_v.map_entity 
    Elementary_symbol_v.elementary_symbol_of_elementary_border_coordinate_heterogeneous_spherical_rho_symbol 
    tag_esr
;;

(** {6 Builder Tag list} *)

let builder_tag_list tag_esr =
  let tag_ele = coerce_up_tag tag_esr in
  Son_elementary_tag_list_by_father_elementary_tag_provider_v.provide tag_ele
;;

(** {6 Builder Tag Datastructure} *)

let builder_tag_uno tag_esr =
  let tag_bui_l = builder_tag_list tag_esr in
  Uno_v.uno_of_list tag_bui_l
;;

(** {6 Building as Builder Tag uno} *)

let building_of_tag tag_esr =
  builder_tag_uno tag_esr
;;

(** {6 Builder Tag tuple} *)

let builder_tag_tuple tag_esr =
  let tag_bui_u = builder_tag_uno tag_esr in
...
let container_of_tag tag_esr =
  let bug = building_of_tag tag_esr in
...
let containee_tag_uno tag_esr =
  container_of_tag tag_esr
;;

let containee_tag_tuple tag_esr =
  let tag_cne_u = containee_tag_uno tag_esr in
  Tuple_v.tuple_of_uno tag_cne_u
;;

(** {6 Border *)

let border_as_list tag_esr =
  let tag_ele = coerce_up_tag tag_esr in
  Elementary_border_by_elementary_tag_provider_v.provide tag_ele
;;

let border_as_duo tag_esr =
  let bor_esr = border_as_list tag_esr in
  Duo_v.duo_of_list bor_esr
;;

let border_as_tuple tag_esr =
  let bor_l = border_as_list tag_esr in
  Tuple_v.tuple_of_list bor_l
;;

let border_as_tuple_list tag_esr =
  let bor_l = border_as_list tag_esr in
...
let fence_as_list tag_esr =
  let tag_ele = coerce_up_tag tag_esr in
  Elementary_fence_by_elementary_tag_provider_v.provide tag_ele
;;

let fence_as_uno tag_esr =
  let fen_esr = fence_as_list tag_esr in
  Uno_v.uno_of_list fen_esr
;;

let fence_as_tuple tag_esr =
  let fen_l = fence_as_list tag_esr in
  Tuple_v.tuple_of_list fen_l
;;

let fence_as_tuple_list tag_esr =
  let fen_l = fence_as_list tag_esr in
  Tuple_list_v.make fen_l
;;


(* generated by ./do_container_as_uno_border_bud_1containee_v.sh elementary_border_coordinate_heterogeneous_spherical_rho esr elementary_fence_units_actual_length eal *)
