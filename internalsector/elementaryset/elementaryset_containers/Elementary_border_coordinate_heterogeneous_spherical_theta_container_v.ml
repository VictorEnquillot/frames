(** {3 Elementary_border_coordinate_heterogeneous_spherical_theta_container_v} *)

(** {6 Documenting_for_container} *)

let documentation () = 
  [
   "Current : FELE:Elementary_border_coordinate_heterogeneous_spherical_theta_container_v";
...
let string_off con_est =
  Uno_v.name 
    Elementary_fence_units_actual_angle_tag_v.string_off
    con_est
;;

let longname con_est =
  Uno_v.name 
    Elementary_fence_units_actual_angle_tag_v.longname
    con_est
;;

let fullname con_est =
  Uno_v.name 
    Elementary_fence_units_actual_angle_tag_v.fullname
    con_est
;;

(** {6 Coercing Up Elementary_border_coordinate_heterogeneous_spherical_theta Tag} *)

let coerce_up_tag tag_est =
  Tag_v.map_entity 
    Elementary_symbol_v.elementary_symbol_of_elementary_border_coordinate_heterogeneous_spherical_theta_symbol 
    tag_est
;;

(** {6 Builder Tag list} *)

let builder_tag_list tag_est =
  let tag_ele = coerce_up_tag tag_est in
  Son_elementary_tag_list_by_father_elementary_tag_provider_v.provide tag_ele
;;

(** {6 Builder Tag Datastructure} *)

let builder_tag_uno tag_est =
  let tag_bui_l = builder_tag_list tag_est in
  Uno_v.uno_of_list tag_bui_l
;;

(** {6 Building as Builder Tag uno} *)

let building_of_tag tag_est =
  builder_tag_uno tag_est
;;

(** {6 Builder Tag tuple} *)

let builder_tag_tuple tag_est =
  let tag_bui_u = builder_tag_uno tag_est in
...
let container_of_tag tag_est =
  let bug = building_of_tag tag_est in
...
let containee_tag_uno tag_est =
  container_of_tag tag_est
;;

let containee_tag_tuple tag_est =
  let tag_cne_u = containee_tag_uno tag_est in
  Tuple_v.tuple_of_uno tag_cne_u
;;

(** {6 Border *)

let border_as_list tag_est =
  let tag_ele = coerce_up_tag tag_est in
  Elementary_border_by_elementary_tag_provider_v.provide tag_ele
;;

let border_as_duo tag_est =
  let bor_est = border_as_list tag_est in
  Duo_v.duo_of_list bor_est
;;

let border_as_tuple tag_est =
  let bor_l = border_as_list tag_est in
  Tuple_v.tuple_of_list bor_l
;;

let border_as_tuple_list tag_est =
  let bor_l = border_as_list tag_est in
...
let fence_as_list tag_est =
  let tag_ele = coerce_up_tag tag_est in
  Elementary_fence_by_elementary_tag_provider_v.provide tag_ele
;;

let fence_as_uno tag_est =
  let fen_est = fence_as_list tag_est in
  Uno_v.uno_of_list fen_est
;;

let fence_as_tuple tag_est =
  let fen_l = fence_as_list tag_est in
  Tuple_v.tuple_of_list fen_l
;;

let fence_as_tuple_list tag_est =
  let fen_l = fence_as_list tag_est in
  Tuple_list_v.make fen_l
;;


(* generated by ./do_container_as_uno_border_bud_1containee_v.sh elementary_border_coordinate_heterogeneous_spherical_theta est elementary_fence_units_actual_angle eaa *)
