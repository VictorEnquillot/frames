(** {3 Elementary_border_coordinate_homogeneous_planar_x_container_v} *)

(** {6 Documenting_for_container} *)

let documentation () = 
  [
   "Current : FELE:Elementary_border_coordinate_homogeneous_planar_x_container_v";
...
let string_off con_epx =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.string_off
    con_epx
;;

let longname con_epx =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.longname
    con_epx
;;

let fullname con_epx =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.fullname
    con_epx
;;

(** {6 Coercing Up Elementary_border_coordinate_homogeneous_planar_x Tag} *)

let coerce_up_tag tag_epx =
  Tag_v.map_entity 
    Elementary_symbol_v.elementary_symbol_of_elementary_border_coordinate_homogeneous_planar_x_symbol 
    tag_epx
;;

(** {6 Builder Tag list} *)

let builder_tag_list tag_epx =
  let tag_ele = coerce_up_tag tag_epx in
  Son_elementary_tag_list_by_father_elementary_tag_provider_v.provide tag_ele
;;

(** {6 Builder Tag Datastructure} *)

let builder_tag_uno tag_epx =
  let tag_bui_l = builder_tag_list tag_epx in
  Uno_v.uno_of_list tag_bui_l
;;

(** {6 Building as Builder Tag uno} *)

let building_of_tag tag_epx =
  builder_tag_uno tag_epx
;;

(** {6 Builder Tag tuple} *)

let builder_tag_tuple tag_epx =
  let tag_bui_u = builder_tag_uno tag_epx in
...
let container_of_tag tag_epx =
  let bug = building_of_tag tag_epx in
...
let containee_tag_uno tag_epx =
  container_of_tag tag_epx
;;

let containee_tag_tuple tag_epx =
  let tag_cne_u = containee_tag_uno tag_epx in
  Tuple_v.tuple_of_uno tag_cne_u
;;

(** {6 Border *)

let border_as_list tag_epx =
  let tag_ele = coerce_up_tag tag_epx in
  Elementary_border_by_elementary_tag_provider_v.provide tag_ele
;;

let border_as_duo tag_epx =
  let bor_epx = border_as_list tag_epx in
  Duo_v.duo_of_list bor_epx
;;

let border_as_tuple tag_epx =
  let bor_l = border_as_list tag_epx in
  Tuple_v.tuple_of_list bor_l
;;

let border_as_tuple_list tag_epx =
  let bor_l = border_as_list tag_epx in
...
let fence_as_list tag_epx =
  let tag_ele = coerce_up_tag tag_epx in
  Elementary_fence_by_elementary_tag_provider_v.provide tag_ele
;;

let fence_as_uno tag_epx =
  let fen_epx = fence_as_list tag_epx in
  Uno_v.uno_of_list fen_epx
;;

let fence_as_tuple tag_epx =
  let fen_l = fence_as_list tag_epx in
  Tuple_v.tuple_of_list fen_l
;;

let fence_as_tuple_list tag_epx =
  let fen_l = fence_as_list tag_epx in
  Tuple_list_v.make fen_l
;;


(* generated by ./do_container_as_uno_border_bud_1containee_v.sh elementary_border_coordinate_homogeneous_planar_x epx elementary_fence_units_actual_length eal *)
