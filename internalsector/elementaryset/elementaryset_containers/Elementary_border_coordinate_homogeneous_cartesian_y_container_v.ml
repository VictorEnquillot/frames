(** {3 Elementary_border_coordinate_homogeneous_cartesian_y_container_v} *)

(** {6 Documenting_for_container} *)

let documentation () = 
  [
   "Current : FELE:Elementary_border_coordinate_homogeneous_cartesian_y_container_v";
...
let string_off con_hcy =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.string_off
    con_hcy
;;

let longname con_hcy =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.longname
    con_hcy
;;

let fullname con_hcy =
  Uno_v.name 
    Elementary_fence_units_actual_length_tag_v.fullname
    con_hcy
;;

(** {6 Coercing Up Elementary_border_coordinate_homogeneous_cartesian_y Tag} *)

let coerce_up_tag tag_hcy =
  Tag_v.map_entity 
    Elementary_symbol_v.elementary_symbol_of_elementary_border_coordinate_homogeneous_cartesian_y_symbol 
    tag_hcy
;;

(** {6 Builder Tag list} *)

let builder_tag_list tag_hcy =
  let tag_ele = coerce_up_tag tag_hcy in
  Son_elementary_tag_list_by_father_elementary_tag_provider_v.provide tag_ele
;;

(** {6 Builder Tag Datastructure} *)

let builder_tag_uno tag_hcy =
  let tag_bui_l = builder_tag_list tag_hcy in
  Uno_v.uno_of_list tag_bui_l
;;

(** {6 Building as Builder Tag uno} *)

let building_of_tag tag_hcy =
  builder_tag_uno tag_hcy
;;

(** {6 Builder Tag tuple} *)

let builder_tag_tuple tag_hcy =
  let tag_bui_u = builder_tag_uno tag_hcy in
...
let container_of_tag tag_hcy =
  let bug = building_of_tag tag_hcy in
...
let containee_tag_uno tag_hcy =
  container_of_tag tag_hcy
;;

let containee_tag_tuple tag_hcy =
  let tag_cne_u = containee_tag_uno tag_hcy in
  Tuple_v.tuple_of_uno tag_cne_u
;;

(** {6 Border *)

let border_as_list tag_hcy =
  let tag_ele = coerce_up_tag tag_hcy in
  Elementary_border_by_elementary_tag_provider_v.provide tag_ele
;;

let border_as_duo tag_hcy =
  let bor_hcy = border_as_list tag_hcy in
  Duo_v.duo_of_list bor_hcy
;;

let border_as_tuple tag_hcy =
  let bor_l = border_as_list tag_hcy in
  Tuple_v.tuple_of_list bor_l
;;

let border_as_tuple_list tag_hcy =
  let bor_l = border_as_list tag_hcy in
...
let fence_as_list tag_hcy =
  let tag_ele = coerce_up_tag tag_hcy in
  Elementary_fence_by_elementary_tag_provider_v.provide tag_ele
;;

let fence_as_uno tag_hcy =
  let fen_hcy = fence_as_list tag_hcy in
  Uno_v.uno_of_list fen_hcy
;;

let fence_as_tuple tag_hcy =
  let fen_l = fence_as_list tag_hcy in
  Tuple_v.tuple_of_list fen_l
;;

let fence_as_tuple_list tag_hcy =
  let fen_l = fence_as_list tag_hcy in
  Tuple_list_v.make fen_l
;;


(* generated by ./do_container_as_uno_border_bud_1containee_v.sh elementary_border_coordinate_homogeneous_cartesian_y hcy elementary_fence_units_actual_length eal *)
