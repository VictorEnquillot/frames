(** {3 Energy_kinetic_value_register_v} *)
...
let documentation () = 
  [
   "Needs: ";
   "Current : VHAR:Energy_kinetic_value_register_v";
   "Needed-by: ";
   "What-is-it: ";
   "How-is-it-done: ";
   "Example: ";
...
let name_of_key tag_eto =
  Tag_v.name Energy_kinetic_symbol_v.name tag_eto
;;

let name_of_value val_eto  =
  let flo = Energy_kinetic_value_v.float_off_energy_kinetic_value val_eto in
...
let retrieve tag_eto =
  ...
  with Failure ("Not_stored:Register_v.retrieve") ->
    failwith ("Not_stored:Energy_kinetic_value_provider_v.retrieve")
;;  
