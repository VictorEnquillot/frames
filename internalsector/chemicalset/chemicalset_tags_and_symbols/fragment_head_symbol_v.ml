...
let nam_mod = "Fragment_head_symbol_v";;
...
let name = function
  | Fhs_t.Fragment_head_halfbridge -> Format.sprintf "%s" "fragment_head_halfbridge"
  | Fhs_t.Block_zerofork_symbol smb_bzf -> 
      Bzfs_v.name smb_bzf
  | Fhs_t.Fragment_zerofork -> Format.sprintf "%s" "fragment_zerofork"
  | Fhs_t.Segment_head_extends_fragment_buried -> Format.sprintf "%s" "segment_head_extends_fragment_buried"
;;

let print ppf = function
  | Fhs_t.Fragment_head_halfbridge -> Format.fprintf ppf "%s" "fragment_head_halfbridge"
  | Fhs_t.Block_zerofork_symbol smb_bzf -> 
      Bzfs_v.print ppf smb_bzf
  | Fhs_t.Fragment_zerofork -> Format.fprintf ppf "%s" "fragment_zerofork"
  | Fhs_t.Segment_head_extends_fragment_buried -> Format.fprintf ppf "%s" "segment_head_extends_fragment_buried"
;;
...
let block_zerofork_symbol_off_fragment_head_symbol = function
  | Fhs_t.Block_zerofork_symbol smb_bzf -> smb_bzf
  | s ->  print_fatal_error
...
let block_zerobifork_symbol_off_fragment_head_symbol smb_frh =
  let smb_bzf = block_zerofork_symbol_off_fragment_head_symbol smb_frh in
...
let is_block_zerofork_symbol_off_fragment_head_symbol = function
  | Fhs_t.Block_zerofork_symbol _ -> true
  | _ -> false
;;
...
let ch_s3 = fragment_head_symbol_of_block_zerotrifork_symbol Bzts_t.Ch_s3;;

(** {12 Upgrading from N_s3 through Block_zerotrifork through Block_zerofork to Fragment_head.} *)

let n_s3 = fragment_head_symbol_of_block_zerotrifork_symbol Bzts_t.N_s3;;

(** {9 Upgrading from Block_zeroquadrifork through Block_zerofork to Fragment_head.} *)

let fragment_head_symbol_of_block_zeroquadrifork_symbol smb_bzq =
...
let c_q = fragment_head_symbol_of_block_zeroquadrifork_symbol Bzqs_t.C_q;;

(** {12 Upgrading from N_q_1p through Block_zeroquadrifork through Block_zerofork to Fragment_head.} *)

let n_q_1p = fragment_head_symbol_of_block_zeroquadrifork_symbol Bzqs_t.N_q_1p;;
...
(** created by ./do_entitysum_symbol_v_ml.sh fragment_head 12 January 2011. *)
