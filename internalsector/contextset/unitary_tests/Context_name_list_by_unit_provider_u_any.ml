...
testing "Context_name_list_by_unit_provider_v
...
(* toplevel 
   #use "Context_name_list_by_unit_provider_u_any.ml";;

*)

open Context_name_list_by_unit_provider_v;;
  ...
  let nam_con_nsl = nam_sec_l @ nam_dom_l @ nam_dba_l @ nam_dbo_l @ nam_iba_l @ nam_ibo_l;; 
  let nam_con_l = List.sort String.compare nam_con_nsl;;

test_number 1 (
List_v.is_once_list_of_list nam_con_l
);;

let fake_l = "user" :: nam_con_l;;
let nam_con_nol = List.filter (fun e -> not (List_v.is_once_of_element_of_list e fake_l)) fake_l;; 
    
test_number 2 (
(nam_con_nol : String.t list ) = 
["user"; "user"]
);;
