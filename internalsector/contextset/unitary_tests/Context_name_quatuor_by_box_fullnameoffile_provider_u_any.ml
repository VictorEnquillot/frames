...
testing "Context_name_quatuor_by_box_fullnameoffile_provider_v
...
(* toplevel 
   #use "Context_name_quatuor_by_box_fullnameoffile_provider_u_any.ml";;

*)

open Context_name_quatuor_by_box_fullnameoffile_provider_v;;

(* Nwchemset *)

let env_var = "NWCF";;
let fnd = Sys.getenv env_var ;;
let fno_box = fnd ^ "/cc_pVTZ.nwc";;
...
let fnd = Filename.dirname fno_box;;
...
let nam_box = Context_name_by_context_fullnameoffile_provider_v.provide fno_box;;
...
let nam_con_t = Context_name_trio_by_fullnameofdirectory_provider_v.provide fno_box;;

test_number 4 (
(nam_con_t : string * string * string ) = 
("nwchem", "skeleton", "external")
);;

let nam_con_q = Quatuor_v.make_of_element_of_trio nam_box nam_con_t;;

test_number 5 (
(nam_con_q : string Quatuor_t.quatuor ) =
  ("cc_pVTZ", "nwchem", "skeleton", "external")
);;

test_number 6 (
nam_con_q = provide fno_box
);;
