...
let wor_sl = List_v.sublist_of_int_of_length_of_list (ind_fra+1) 2 wor_l ;;

test_number 4 (
(wor_sl : string list ) = 
["entrysector"; "userinput"]
);;
...
let nam_con_d = Duo_v.duo_of_list nam_con_l;;

test_number 5 (
(nam_con_d : string Duo_t.duo ) =
("user", "entry")
);;

test_number 6 (
nam_con_d = provide fnd
);;
...
(wor_l : string list ) =
  ["home"; "colonna"; "sources"; "ocaml_top"; "setup"; "frames";
   "internalsector"; "elementaryset"; "elementaryset_builder_trees"]
);;
