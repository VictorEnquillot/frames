...
testing "Context_name_inputbox_by_inputbox_fullnameoffile_provider_v
...
(* toplevel 
   #use "Context_name_inputbox_by_inputbox_fullnameoffile_provider_u_any.ml";;

*)

open Context_name_inputbox_by_inputbox_fullnameoffile_provider_v;;

(* Localinput *)

let env_var = "LOIF";;
let fnd = Sys.getenv env_var ;;
let fno_ibo = fnd ^ "/Point_a_print.loc";;
...
test_number 2 (
 List.mem fno_ibo fno_ibo_l
);;

let wor_l = String_v.split_of_character_of_string '/' fno_ibo;;
...
let nof_ibo = List_v.last_element_off_list wor_l;;
